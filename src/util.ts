 ...
 */
export const dateReviver: JsonParseReviver = (key, value) =>
...
const isIsoDate = (value: any): boolean =>
    typeof value === 'string' && isoDateRegExp.test(value);

export const shallowMerge = <T, U>(source1: T, source2: U): T & U =>
 ...
 */
export const combineProcedures = <A extends any[]>(...procedures: ((...args: A) => void)[]): (...args: A) => void =>
    (...args) => procedures.forEach(procedure => procedure(...args));
...
export const handleResultWith = <R, A extends any[]>(source: (...args: A) => Promise<R>, handle: (result: R) => MaybePromiseVoid) => async (...args: A): Promise<R> => {
...
export const decorateFirstArg = <A, R>(func: (firstArg: A) => R, decorator: (firstArg: A) => A): (firstArg: A) => R =>
    firstArg => func(decorator(firstArg));
